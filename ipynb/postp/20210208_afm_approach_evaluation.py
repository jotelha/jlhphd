...
# %%
uri_df = await get_df_by_filtered_query(query)
...
parameter_sets_df = res_df[['shape','concentration']].drop_duplicates()
restricted_parameter_sets_df = parameter_sets_df[parameter_sets_df['shape'] == 'monolayer']
...
plt.savefig('monolayer_v_10_m_per_s_c_0.25_to_2.75_per_nm_sq.svg', dpi=300)
...
parameter_sets_df = res_df[['shape','concentration']].drop_duplicates()
restricted_parameter_sets_df = parameter_sets_df[parameter_sets_df['shape'] == 'hemicylinders']
...
plt.savefig('hemicylinders_v_10_m_per_s_c_0.5_to_1.75_per_nm_sq.svg', dpi=300)
...
for _, row in parameter_sets_df[parameter_sets['concentration'] == 0.005].iterrows():
    sub_df = res_df[ (res_df['concentration'] == row['concentration']) & (res_df['shape'] == row['shape'])]
...
for _, row in parameter_sets_df[parameter_sets['concentration'] == 0.01].iterrows():
    sub_df = res_df[ (res_df['concentration'] == row['concentration']) & (res_df['shape'] == row['shape'])]
